...
class Ball {
  ...
  public:
    ...
    Ball(double x,double y,double z,double _r=1.0):x_(x),y_(y),z_(z),r_(_r){d = 2*r_;}
    ...
    double operator[](int i){
      switch (i){
        ...
        case 2:
          ...
          break;
        default:
          std::cout<<"[CLASS BALL] wrong index!";
          break;
      }
    }
...
class Cube {
  ...
  public:
    ...
    Cube(double x,double y,double z,double _size=1.0):x_(x),y_(y),z_(z),size(_size){}
    ...
    double operator[](int i){
      switch (i){
        ...
        case 2:
          ...
          break;
        default:
          std::cout<<"[CLASS Cube] wrong index!";
          break;
      }
    }
...
class Ellipsoid {
  ...
  public:
    ...
    Ellipsoid(double x,double y,double z,double _rx,double _ry,double _rz,Eigen::Matrix3d _R):
    x_(x),y_(y),z_(z),
    ...
    Ellipsoid(double x,double y,double z,double _rx,double _ry,double _rz):
    x_(x),y_(y),z_(z),
    rx(_rx),ry(_ry),rz(_rz){
        c.x() = x_;
        c.y() = y_;
...
class Visualization {
  ...
  void setMarkerColor(visualization_msgs::Marker& marker,
    ...
    switch (color) {
      ...
      case mediumSlateBlue:
        marker.color.r = 123.0 / 255.0;
        marker.color.g = 104.0 / 255.0;
        marker.color.b = 238.0 / 255.0;
        break;
      case mediumVioletRed:
        marker.color.r = 199.0 / 255.0;
        marker.color.g = 21.0 / 255.0;
        marker.color.b = 133.0 / 255.0;
        break;
      case hotPink:
 ...
 public:
  ...
  void visualize_a_cube(const CENTER& c,
                        const double& size,
                        const TOPIC & topic,
      ...
      setMarkerScale(marker,size,size,size);
  ...
  void visualize_a_cube(const CENTER& c,
      ...
      setMarkerPose(marker,c[0],c[1],c[2],R_);
      marker.header.stamp = ros::Time::now();
      publisher_map_[topic].publish(marker);
}
  ...
  void visualize_a_cube(const CENTER& c,
      ...
      setMarkerColor(marker,color,a);
      setMarkerScale(marker,size[0],size[1],size[2]);
      setMarkerPose(marker,c[0],c[1],c[2],q_);
      marker.header.stamp = ros::Time::now();
      publisher_map_[topic].publish(marker);
}
  ...
  template <class CENTER, class TOPIC>
  void visualize_cubes(const CENTER& centers,
                        double size,
                        const TOPIC & topic,
                        const Color color = mediumSlateBlue,
                        const double a = 0.5){

      // ROS_WARN("[visualization] in visualize_cubes with size");
      auto got = publisher_map_.find(topic);
      if(got == publisher_map_.end()){
      ...
      // ROS_WARN("[visualization] centers.size : %d",centers.size());
  ...
  void visualize_cubes(const CUBES& cubes,
      ...
      setMarkerColor(marker,color,a);
      visualization_msgs::MarkerArray marker_array;
      marker_array.markers.reserve(cubes.size() + 1);
      marker.action = visualization_msgs::Marker::DELETEALL;
      marker_array.markers.push_back(marker);
      marker.action = visualization_msgs::Marker::ADD;
      for (const auto& cube : cubes){
        setMarkerPose(marker,cube[0],cube[1],cube[2]);
  ...
  void visualize_cylinder(const Eigen::Vector3d & p_, 
                          const Eigen::Quaterniond & q_, 
                          const Eigen::Vector3d & size, 
                          const TOPIC& topic, 
                          const Color& color = blue, 
                          const double a = 1)
  {
    ...
    visualization_msgs::Marker cylinder_msg;
    cylinder_msg.type = visualization_msgs::Marker::CYLINDER;
    cylinder_msg.action = visualization_msgs::Marker::ADD;
    cylinder_msg.header.frame_id = "world";
    cylinder_msg.id = 0;
    setMarkerPose(cylinder_msg, p_[0], p_[1], p_[2], q_);

    cylinder_msg.scale.x = size(0);
    cylinder_msg.scale.y = size(1);
    cylinder_msg.scale.z = size(2);
    setMarkerColor(cylinder_msg, color, a);
    publisher_map_[topic].publish(cylinder_msg);
  }
  ...
  void visualize_arrow(const Eigen::Vector3d p0, const Eigen::Vector3d& p1, const TOPIC& topic, const Color& color = blue) {
    ...
    publisher_map_[topic].publish(arrow_msg);
      ...
      template <class TOPIC>
  void visualize_arrow(const Eigen::Vector3d p0, const Eigen::Vector3d& p1,const double & shaft_diameter, const TOPIC& topic, const Color& color = blue) {
    auto got = publisher_map_.find(topic);
    if (got == publisher_map_.end()) {
      ros::Publisher pub = nh_.advertise<visualization_msgs::Marker>(topic, 10);
      publisher_map_[topic] = pub;
    ...
    arrow_msg.points.resize(2);
    setMarkerPose(arrow_msg, 0, 0, 0);
    setMarkerScale(arrow_msg, shaft_diameter, shaft_diameter * 2.5, 0);
    setMarkerColor(arrow_msg, color);
    arrow_msg.points[0].x = p0[0];
    arrow_msg.points[0].y = p0[1];
    arrow_msg.points[0].z = p0[2];
  ...
  void visualize_arrows(const ARROWS& arrows, const TOPIC& topic) {
    ...
    setMarkerColor(arrow_msg, mediumVioletRed);
    ...
    template <class ARROWS, class TOPIC>
  // ARROWS: pair<Vector3d, Vector3d>
  void visualize_arrows(const ARROWS& arrows, const TOPIC& topic,const  double & shaft_diameter,const Color color) {
    auto got = publisher_map_.find(topic);
    if (got == publisher_map_.end()) {
      ros::Publisher pub =
          nh_.advertise<visualization_msgs::MarkerArray>(topic, 10);
